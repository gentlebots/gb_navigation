...
public:
  ...
  void init()
  {
    ...
    // problem_expert_->addPredicate(plansys2::Predicate("(robot_at c3po " + robot_location_["c3po"] +")"));
    // problem_expert_->addPredicate(plansys2::Predicate("(battery_full c3po)"));
    // problem_expert_->addPredicate(plansys2::Predicate("(robot_at bb8 " + robot_location_["bb8"] +")"));
  ...
  void init_knowledge()
  {
    ...
    problem_expert_->addPredicate(plansys2::Predicate("(is_pantry_zone pantry_zone)"));
    ...
    //  plansys2::Goal(
    //    "(and(car_cookingd car_1) (car_cookingd car_2) (car_cookingd car_3))"));
  }
  ...
  void generate_new_mission()
  {
    ...
    // RCLCPP_INFO_STREAM(get_logger(), "I will coock " << num_dishes << " dishes");
  ...
  void step()
  {
    ...
    RCLCPP_INFO(get_logger(), "Battery level = [%lf]", battery_level_["r2d2"]); //, battery_level_["c3po"], battery_level_["bb8"]);
    ...
    if (!executor_client_->execute_and_check_plan()) {
      ...
      if (result.has_value()) {
        ...
        // RCLCPP_INFO_STREAM(get_logger(), "\tStart: " << start_stamp.seconds());
        // RCLCPP_INFO_STREAM(get_logger(), "\tStop: " << status_stamp.seconds());
        // RCLCPP_INFO_STREAM(get_logger(), "\tcompletion: " << action_info.completion << "  [" << action_info.message_status << "]");
        // RCLCPP_INFO_STREAM(get_logger(), "\tDuration: " << (status_stamp - start_stamp).seconds() << "secs");
        }
      } else {
      ...
      if (!found_r2d2) {
        RCLCPP_INFO_STREAM(get_logger(), "Restoring robot location to [" << robot_location_["r2d2"] + "]");
        problem_expert_->addPredicate(plansys2::Predicate("(robot_at r2d2 " + robot_location_["r2d2"] +")"));
      }
      // if (!found_c3po) {
      //   RCLCPP_INFO_STREAM(get_logger(), "Restoring robot location to [" << robot_location_["c3po"] + "]");
      //   problem_expert_->addPredicate(plansys2::Predicate("(robot_at c3po " + robot_location_["c3po"] +")"));
      // }
      // if (!found_bb8) {
      //   RCLCPP_INFO_STREAM(get_logger(), "Restoring robot location to [" << robot_location_["bb8"] + "]");
      //   problem_expert_->addPredicate(plansys2::Predicate("(robot_at bb8 " + robot_location_["bb8"] +")"));
      // }
